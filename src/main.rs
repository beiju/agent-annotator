...
// async fn experiment_preview_video(db: AnnotatorDbConn, _user: AdminUser, experiment_id: i32, _video_filename: &str) -> WebResult<response::status::Custom<ByteStream![Vec<u8>]>> {
//     use opencv::prelude::*;
//
//     let experiment = db.run(move |c| {
//         experiments::get_experiment(c, experiment_id)
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    rocket::build()
        .mount("/", routes![
            ...
            experiment_preview, list_save, list_set_status
        ])
